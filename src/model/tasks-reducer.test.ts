...
test('correct task should change its title', () => {
    const endState = tasksReducer(
        startState,
        changeTaskTitleAC({todolistId: 'todolistId2', taskId: '2', newTitle: "clining"})
    )

    expect(endState.todolistId2[1].title).toBe("clining")
    expect(startState.todolistId2[1].title).toBe("milk")

})
test('All tasks must be correctly deleted', () => {
    const endState = tasksReducer(
        startState,
        deleteAllTaskAC({todolistId: 'todolistId2'})
    )
