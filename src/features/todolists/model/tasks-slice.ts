...
export const tasksSlice = createAppSlice({
  ...
  reducers: (create) => ({
    ...
    changeTaskTitleTC: create.asyncThunk(
      async (arg: { task: DomainTask; newTitle: string }, thunkAPI) => {
        try {
          const model: UpdateTaskModel = {
          ...
          const res = await tasksApi.updateTask({
            taskId: arg.task.id,
            todolistId: arg.task.todoListId,
            model: model,
          })
          return res.data.data.item
        } catch (error) {
...
export const { changeTaskTitleAC, fetchTasksTC, createTaskTC, deleteTaskTC, changeTaskStatusTC, changeTaskTitleTC } =
  tasksSlice.actions
